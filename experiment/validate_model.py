...
# Spectral shift config for OptimalSpectralBias (experiment folder uses pure math approach)
DEFAULT_SPECTRAL_SHIFT_CONFIG = {
    "sample_rate": SAMPLING_RATE_HZ,
    "n_channels": 32,
    "n_odors": 7,
    "band_width_hz": DEFAULT_CONFIG.get("spectral_shift_band_width_hz", 2),
}
...
def create_spectral_shift(config: Dict[str, Any], device: torch.device):
    """Create an OptimalSpectralBias module from config.

    The experiment folder uses OptimalSpectralBias - a pure math approach that computes
    optimal per-odor bias from UNet output vs target PSD difference.
    """
    shift = OptimalSpectralBias(
        n_channels=config["n_channels"],
        n_odors=config["n_odors"],
        sample_rate=config["sample_rate"],
        band_width_hz=config.get("band_width_hz", 2),
    )
...
def load_checkpoint(
    ...
    # Check if OptimalSpectralBias was trained with different n_odors
    # OptimalSpectralBias stores bias as odor_bias with shape [n_odors, n_bands]
    ...
    if "spectral_shift_fwd" in checkpoint:
        ...
        if "odor_bias" in ss_state:
            spectral_n_odors_fwd = ss_state["odor_bias"].shape[0]
            if spectral_n_odors_fwd != n_odors:
                print(f"  Warning: OptimalSpectralBias fwd was trained with {spectral_n_odors_fwd} conditions (model has {n_odors})")
    if "spectral_shift_rev" in checkpoint:
        ss_state = checkpoint["spectral_shift_rev"]
        if "odor_bias" in ss_state:
            spectral_n_odors_rev = ss_state["odor_bias"].shape[0]
            if spectral_n_odors_rev != n_odors:
                print(f"  Warning: OptimalSpectralBias rev was trained with {spectral_n_odors_rev} conditions (model has {n_odors})")
